...
export function getTagColor(colorName: string): TagColor {
  return TAG_COLORS.find((c) => c.name === colorName) ?? TAG_COLORS[TAG_COLORS.length - 1]
}

export function getFirstUnusedColor(usedColorNames: string[]): TagColor {
  return TAG_COLORS.find((c) => !usedColorNames.includes(c.name)) ?? TAG_COLORS[0]
}

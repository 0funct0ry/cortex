...
const getMethodBg = (method: string) => {
  ...
  return 'bg-bg-muted border-border-default'
}
...
const MethodSelector: React.FC<MethodSelectorProps> = ({ method, onChange }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <Tooltip content="Change HTTP method or protocol">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className={`w-[72px] h-[30px] rounded-md border flex items-center justify-between px-2 transition-colors ${getMethodBg(method)} ${getMethodColor(method)} text-sm font-semibold`}
        >

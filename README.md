...
Three living reference stories live under **Design System** in the Storybook sidebar:

| Story | Path | What it shows |
|---|---|---|
...
| **CodeEditor** | JsonEditable, YamlEditable, JavaScriptEditable, MarkdownEditable, XmlEditable, HtmlEditable, ReadOnly, WithVariableAutoComplete, WithWordWrap | All 6 language modes + read-only + `{{variable}}` autocomplete (fixture `resolvedVariables`) + word-wrap; no store dependencies; `language` select argType |
| **ContextMenu** | Default, WithSeparators, WithDisabledItems, WithShortcuts, WithDangerItem, WithSubmenu | Portal menu; `inline:false` iframe isolation; `play()` on Default uses ArrowDown+Escape keyboard nav; WithDangerItem `play()` clicks item and asserts `onClose` spy; all separator objects carry `label:''` for type safety |
| **CommandPaletteModal** | Open, SearchMode, EmptyResults, CommandMode | Full-screen palette; stores seeded via `beforeEach` (`useUIStore.openCommandPalette/openCommandMode`, `useCollectionStore.setState`); SearchMode types "pay" and asserts result list; EmptyResults asserts "no results" empty state |
| **CreateExampleModal** | Open, WithCustomName, Closed | Portal modal driven by `uiStore.createExampleModal`; `beforeEach` seeds `isOpen:true`; `play()` on WithCustomName clears name input, types custom text, asserts Save enabled |
| **CreateWorkspaceModal** | Open, Closed, WithNameFilled, CancelCloses | Props-driven modal; `tauriMock` for `pick_directory` / `create_workspace`; Create button stays disabled until both name + path filled; `play()` on CancelCloses asserts `onClose` spy |
| **GenerateCodeModal** | Open, cURL Target, Fetch Target, Python Target, Closed | Portal modal; stores seeded via `beforeEach`; `tauriMock` for `load_request` + `get_resolved_variables` with fixture POST request; language `<select>` verified in play |
| **GenerateDocsModal** | Open, HtmlTab, MarkdownTab, OtherFormatsTab, Closed | 3-tab docs export modal; `beforeEach` seeds `uiStore.generateDocsModal`; `tauriMock` for all generation commands; MarkdownTab + OtherFormatsTab `play()` clicks tab and asserts `aria-selected` |
| **ImportCollectionDialog** | Open, BundleFormat, PreviewStep, ConflictStep, ImportError, Closed | Multi-step ZIP/bundle import dialog driven by `uiStore.importCollectionDialog`; `tauriMock` for pick/preview/extract commands; ImportError story returns error response to test inline error display |
| **ImportFolderDialog** | Open, Closed, PreviewStep, WithParseErrors, SummaryStep | Props-driven multi-step folder import; `tauriMock` for `pick_directory` + `preview_import_folder` + `import_folder`; WithParseErrors fixture includes a YAML parse error row locked to Skip |
| **ImportModal** | Open, Closed, Select ZIP Format, Select Bundle Format | Format-selection portal; `play()` on Select ZIP/Bundle clicks card and asserts `uiStore.importCollectionDialog.format` equals expected value |
| **NewRequestDialog** | Open, Type: GraphQL/gRPC/WebSocket/From cURL, Validation (name required/filled), Create submit flow, CancelCloses, Closed | Props-driven portal dialog with `TabsProvider` decorator; `tauriMock` for `create_request` / `save_request` / `load_collection`; `fireEvent.change` for controlled input; Create button disabled until name non-empty; `waitFor` asserts `onClose` spy after async IPC chain |
